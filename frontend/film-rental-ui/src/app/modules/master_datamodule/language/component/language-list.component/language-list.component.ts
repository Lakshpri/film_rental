...
export class LanguageListComponent implements OnInit {
  ...
  validate(): boolean {
    if (!this.formData.name?.trim()) { this.error = 'Language name is required.'; return false; }
    return true;
  }
  ...
  save(): void {
    ...
    const call = this.editItem ? this.svc.update(this.editItem.languageId, { name: this.formData.name.trim() }) : this.svc.create({ name: this.formData.name.trim() });
    call.subscribe({
      next: () => { this.successMsg = `Language ${this.editItem ? 'updated' : 'created'}!`; this.closeModal(); this.load(); setTimeout(() => this.successMsg = '', 3000); },
  ...
  delete(item: any): void {
    ...
    this.svc.delete(item.languageId).subscribe({
      next: () => { this.successMsg = 'Language deleted!'; this.load(); setTimeout(() => this.successMsg = '', 3000); },
      error: (e: any) => { this.error = formatBackendError(e); }
    });
  }
